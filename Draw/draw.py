...
# Original file: D:\codetest\rlpp_ori\Draw\draw.py
...
class Canvas:
    ...
    def transform_abs_coord(self, point):
        ...
        return np.array([x, self.canvas_height - y], dtype=np.int64)
    ...
    def draw_arrow(self, start, direction, length, color, transform_length=True):
        ...
        end = start + (length * self.transform_rel_coord(direction, scale=transform_length)).astype(np.int64)
