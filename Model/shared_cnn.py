...
# Original file: D:\codetest\rlpp_ori\Model\shared_cnn.py
...
class StateModel:
    def __init__(self, eval_model: bool, size_splits, n_filters=5):
        ...
        self.obs = # TODO: Remove placeholder, use direct input
tf.Variable(tf.float64, (None, self.d_obs, 1), name='obs')
        self.goal = # TODO: Remove placeholder, use direct input
tf.Variable(tf.float64, (None, d_goal), name='goal')
        conv = tf.keras.layers.Conv1D(self.obs, n_filters, kernel_size=5, strides=2,
...
class ActorModel:
    def __init__(self, special_info, eval_model: bool):
        ...
        for n_layer in special_info['actor_n_layers']:
            layer = tf.keras.layers.Dense(layer, n_layer, activation=tf.nn.relu, trainable=eval_model,
                                    kernel_initializer=tf.keras.initializers.RandomNormal(0., .1),
                                    bias_initializer=tf.constant_initializer(0.1))
        self.out = tf.keras.layers.Dense(layer, 1, activation=tf.nn.tanh, trainable=eval_model,
                                   kernel_initializer=tf.keras.initializers.RandomNormal(0., .1),
                                   bias_initializer=tf.constant_initializer(0.1))
    ...
    def __init__(self, action, special_info: dict, eval_model: bool):
        ...
        for n_layer in special_info['critic_n_layers']:
            layer = tf.keras.layers.Dense(layer, n_layer, activation=tf.nn.relu, trainable=eval_model,
                                    kernel_initializer=tf.keras.initializers.RandomNormal(0., .1),
                                    bias_initializer=tf.constant_initializer(0.1))
        self.out = tf.keras.layers.Dense(layer, 1, trainable=eval_model,
                                   kernel_initializer=tf.keras.initializers.RandomNormal(0., .1),
                                   bias_initializer=tf.constant_initializer(0.1))
    ...
    def __init__(self, n_actions, special_info: dict, eval_model: bool):
        ...
        for n_layer in special_info['actor_n_layers']:
            layer = tf.keras.layers.Dense(layer, n_layer, activation=tf.nn.relu, trainable=eval_model,
                                    kernel_initializer=tf.keras.initializers.RandomNormal(0., .1),
                                    bias_initializer=tf.constant_initializer(0.1))
        self.out = tf.keras.layers.Dense(layer, n_actions, activation=tf.nn.softmax, trainable=eval_model,
                                   kernel_initializer=tf.keras.initializers.RandomNormal(0., .1),
                                   bias_initializer=tf.constant_initializer(0.1))
    ...
    def __init__(self, n_actions, special_info: dict, eval_model: bool):
        ...
        for n_layer in special_info['critic_n_layers']:
            layer = tf.keras.layers.Dense(layer, n_layer, activation=tf.nn.relu, trainable=eval_model,
                                    kernel_initializer=tf.keras.initializers.RandomNormal(0., .1),
                                    bias_initializer=tf.constant_initializer(0.1))
        self.out = tf.keras.layers.Dense(layer, n_actions, trainable=eval_model,
                                   kernel_initializer=tf.keras.initializers.RandomNormal(0., .1),
                                   bias_initializer=tf.constant_initializer(0.1))
