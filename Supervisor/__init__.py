...
# Original file: D:\codetest\rlpp_ori\Supervisor\__init__.py
# Migration may not be complete. 
# Please review TODO comments.
# ===================================
import tensorflow as tf

# 强制启用 TF 2.x Eager Execution
if hasattr(tf, 'compat') and hasattr(tf.compat.v1, 'enable_eager_execution'):
    try:
        tf.compat.v1.enable_eager_execution()
    except:
        pass  # 已经启用

print(f"[Supervisor] Eager Execution: {tf.executing_eagerly()}")
...
class SupervisorDispatch:
    ...
    def _model_info(self, **model_kwargs):  # 独立+共享
        """model info"""
        tf.compat.v1.reset_default_graph()        # 复原tf避免重名
