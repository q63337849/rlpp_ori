...
# Original file: D:\codetest\rlpp_ori\Scene\__init__.py
...
import os
import numpy as np
from Env.flight_with_dynamic_obstacles import Scenairo
...
class ScenarioLoader:
    ...
    def load_scene(self, obs_id, task_id=None, data_source=D_RANDOM, percentage=1.):
        ...
        if os.path.exists(circle_file):
            self.obss = [obs.astype(np.float64) for obs in np.load(circle_file, allow_pickle=True)]
            ...
            self.obss = [obs.astype(np.float64) for obs in np.load(line_file, allow_pickle=True)]
    ...
    def load_task(self, task_id):
        ...
        return list(zip(np.load(init_p_file, allow_pickle=True).astype(np.float64),
                        np.load(init_d_file, allow_pickle=True).astype(np.float64),
                        np.load(goal_p_file, allow_pickle=True).astype(np.float64),
                        np.load(goal_d_file, allow_pickle=True).astype(np.float64)))
    ...
    def random_scene(self):
        ...
        return Scenairo(tsks[0][tsk_id], tsks[1][tsk_id], tsks[2][tsk_id], tsks[3][tsk_id], obs.astype(np.float64))
